...
def calculate_returns(rewards, discount_factor, normalize = True, device = 'cpu'):
    returns = []
    R = 0
...
def train(env, policy, optimizer, discount, device = 'cpu'):
    policy.train()
    ...
    returns = calculate_returns(rewards, discount, device=device)
    policy_loss, value_loss = update_policy(returns, log_prob_actions, values, optimizer)
    return policy_loss, value_loss, episode_reward


if __name__=='__main__':
    train_env = gym.make('MELANIE')
    test_env = gym.make('MELANIE')
    ...
    for episode in range(1, MAX_EPISODES + 1):
        policy_loss, critic_loss, train_reward = train(train_env, policy, optimizer, DISCOUNT_FACTOR, device=device)
